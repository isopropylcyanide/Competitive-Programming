...
def can_be_formed(x,n,on_table):
	...
	for i in xrange(sol+1):
     		for j in xrange(n+1):
         		...
         		if i >= on_table[j-1]:
           			subset[i][j] = subset[i][j] or subset[i - on_table[j-1]][j-1];
   	return subset [x][n]
...
def check_onTable(on_table,n):
	...
	while i:
		if can_be_formed(i,len(on_tabsle),on_table):
			game_count+=1
		else:
			inf=0
			break
	...
	if k==0:
		...
		if 1 in missing:
			...
			on_table= [x for x in xrange(1,n+1) if x not in missing]
				...
				#now for each number on the given table, check if it can be calculated somehow
				#(subset sum problem)
